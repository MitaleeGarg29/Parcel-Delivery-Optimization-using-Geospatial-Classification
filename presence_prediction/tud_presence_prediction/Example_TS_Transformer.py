...
version = "0"                       # local trained version of the model; needed for prediction and evaluation


if action == "train":

    test_presence_prediction = presence_prediction(
        model_file="Transformer_V2_flatten",
        ...
        extrapolation="none",       
        epochs=20                   # examplary value; sensical values depend on the performance of the performance of a validation split, for which large amounts of reliable data are crucial.
    )
    ...
    # Exemplary settings: window_size_step=sequence_size, window_offset_step=1 (maximum variations), prediction_length_step=prediction_length_max=1 (must match time shift).
