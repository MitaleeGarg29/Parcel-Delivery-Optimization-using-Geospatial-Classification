...
action = "predict with download"                    # action to perform (see below)
version = "1"                       # local trained version of the model; needed for prediction and evaluation
...
if action == "train":
    ...
    test_presence_prediction = presence_prediction(
        ...
        data_time_shift="1:0",      # ensure that one version of a model always uses the same shift for training, predictions, and evaluation
        batch_size=70,              # should be adapted to the accelerator
        sequence_size=[16,],        
        stride_size=8,              

        logging_mode="full",        # may be switched to "text" if image generation causes issues on servers
        extrapolation="none",       
        epochs=60                   # examplary value; sensical values depend on the performance of the performance of a validation split, for which large amounts of reliable data are crucial.
    )
    ...
    test_presence_prediction = presence_prediction(
        ...
        # data below is purely examplary
    ...
    # Examplary settings: window_size_step=sequence_size, window_offset_step=1 (maximum variations), prediction_length_step=prediction_length_max=1 (must match time shift).
