...
class Pew_LSTM_layer_TS(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, weather_size: int):
        super(Pew_LSTM_layer_TS, self).__init__()
        ...
        for d in range(day_size):
            ...
            for t in range(seq_size):
                ...
                if d < 14 * 2:
                    ...
                    h_m = x_input[d - 14 * 2, t, :].unsqueeze(0).t()

                #x = x_input[d, t, :].unsqueeze(0).t()  # [input_dim, 1]
                #weather_t = x_weather[d, t, :].unsqueeze(0).t()  # [weather_dim, 1]
                x = x_input[0:d, t, :].t()  # [input_dim, 1]
                weather_t = x_weather[0:d, t, :].t()  # [weather_dim, 1]
                ...
                h_output[1:d+1, t] = h_next.t()
                c_output[1:d+1, t] = c_next.t()
...
class Pew_LSTM_TS(nn.Module):
    # timemode: 0 for day, 1 for week, 2 for
    def __init__(self, in_dim: int, hidden_dim: int, weather_dim: int, window_size: int = 30):
        super(Pew_LSTM_TS, self).__init__()
        self.hidden_dim = hidden_dim
        self.window_size = window_size
        self.lstm1 = Pew_LSTM_layer_TS(in_dim * window_size, hidden_dim, weather_dim * window_size)
        self.lstm2 = Pew_LSTM_layer_TS(hidden_dim, hidden_dim, weather_dim * window_size)
