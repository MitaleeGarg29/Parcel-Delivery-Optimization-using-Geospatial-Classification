...
class Pew_LSTM_layer_shift_longmem(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, weather_size: int, day_shift=1):
        super(Pew_LSTM_layer_shift_longmem, self).__init__()
        ...
        for d in range(day_size):
            ...
            for t in range(seq_size):
                ...
                if d < 1:
                    ...
                    h_d = x_input[d - (1 + self.day_shift), t, :].unsqueeze(0).t()

                # week
                if d < 7:
                    h_w = torch.zeros(1, self.input_size,device=x_input.device).t()
                else:
                    h_w = x_input[d - (7 + self.day_shift), t, :].unsqueeze(0).t()

                # month
                if d < 14 * 2:
                    h_m = torch.zeros(1, self.input_size,device=x_input.device).t()
                else:
                    h_m = x_input[d - (30 + self.day_shift), t, :].unsqueeze(0).t()

                #x = x_input[d, t, :].unsqueeze(0).t()  # [input_dim, 1]
                #weather_t = x_weather[d, t, :].unsqueeze(0).t()  # [weather_dim, 1]
                x = x_input[d - self.day_shift, t, :].unsqueeze(0).t()  # [input_dim, 1]
                weather_t = x_weather[d - self.day_shift, t, :].unsqueeze(0).t() # [weather_dim, 1]
...
class Pew_LSTM_shift_longmem(nn.Module):
    # timemode: 0 for day, 1 for week, 2 for
    def __init__(self, in_dim: int, hidden_dim: int, weather_dim: int, window_size: int = 30, day_shift=1):
        super(Pew_LSTM_shift_longmem, self).__init__()
        self.day_shift = day_shift
        self.hidden_dim = hidden_dim
        self.window_size = window_size
        self.lstm1 = Pew_LSTM_layer_shift_longmem(in_dim * window_size, hidden_dim, weather_dim * window_size, day_shift=day_shift)
        self.lstm2 = Pew_LSTM_layer_shift_longmem(hidden_dim, hidden_dim, weather_dim * window_size, day_shift=day_shift)
