...
# Suppress specific warnings related to no positive samples in the target
warnings.filterwarnings("ignore", category=UserWarning, message="No positive samples found in target, recall is undefined")
...
class Transformer_V2_regressive(pl.LightningModule):
    ...
    def __init__(self, future_unknown_feature_count, future_known_covariate_feature_count, input_dim_3, hidden_dim=10, classification_layers=1, positional_encoding="time2vec", add_day_feature=False, transformer_head_count=4, transformer_linear_dim=2048, transformer_layer_count=6, transformer_activiation_func="relu", transformer_layer_norm_first=False, transformer_dropout=0.1, use_combined_loss=False, secondary_loss_func=torch.nn.MSELoss, secondary_loss_weight=0.5, training_mode="auto-regressive", change_to_auto_regression=-1, prediction_step_length=(0,26), prediction_total_length=1, use_mask="custom step", pos_weight=1, empty_slot_value=-1, use_optimizer="AdamW", optimizer_beta1=None, optimizer_beta2=None, weight_decay=None, fixed_learning_rate=0.001, log_LR=True, use_LR_scheduler=True, LR_scheduler_min=0.000000175, LR_scheduler_max=0.00009, LR_scheduler_convergence=0.000016, LR_increase_phase_percentage=0.05):
        ...
        self.LR_increase_phase_percentage = LR_increase_phase_percentage
                # Recall and AUC-PR for binary classification
        self.train_recall = torchmetrics.Recall(task="binary", average='macro')  # Remove compute_on_step
        self.train_auc_pr = torchmetrics.AveragePrecision(task="binary")  # AUC-PR metric

        self.val_recall = torchmetrics.Recall(task="binary", average='macro')  # Remove compute_on_step
        self.val_auc_pr = torchmetrics.AveragePrecision(task="binary")  # AUC-PR metric
    ...
    def training_step(self, batch, batch_idx):
        ...
        probas = torch.sigmoid(filtered_pred)

        # Update and log recall and AUC-PR
        self.train_recall(probas, filtered_target.int())
        self.train_auc_pr(probas, filtered_target.int())

        # Logging the metrics
        self.log('train_recall', self.train_recall, on_step=False, on_epoch=True, prog_bar=True)
        self.log('train_auc_pr', self.train_auc_pr, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Update and log recall and AUC-PR
        self.val_recall(probas, filtered_target.int())
        self.val_auc_pr(probas, filtered_target.int())

        # Logging the metrics
        self.log('val_recall', self.val_recall, on_step=False, on_epoch=True, prog_bar=True)
        self.log('val_auc_pr', self.val_auc_pr, on_step=False, on_epoch=True, prog_bar=True)
