...
class Transformer_V2_regressive_c(pl.LightningModule):
    """
    ...
    def __init__(self, future_unknown_feature_count, future_known_covariate_feature_count, input_dim_3, hidden_dim=10, use_layer_norm=False, positional_encoding="time2vec", add_day_feature=False, transformer_head_count=4, transformer_linear_dim=2048, transformer_layer_count=6, transformer_activiation_func="relu", transformer_layer_norm_first=False, transformer_dropout=0.1, use_combined_loss=False, training_mode="auto-regressive", change_to_auto_regression=-1, prediction_step_length=(0,26), prediction_total_length=1, use_mask="custom step", pos_weight=1, empty_slot_value=-1, use_optimizer="AdamW", optimizer_beta1=None, optimizer_beta2=None, weight_decay=None, fixed_learning_rate=0.001, log_LR=True, use_LR_scheduler=True, LR_scheduler_min=0.000000175, LR_scheduler_max=0.00009, LR_scheduler_convergence=0.000016, LR_increase_phase_percentage=0.05):
        ...
        super(Transformer_V2_regressive_c, self).__init__()
        extra_dim = 0
        ...
        if positional_encoding == "time2vec":
            ...
            transformer_head_count = 3 # TODO: more dynamic approach
        else:
            raise ValueError("Input encoding not recognized.")
        
        decoder_layer = torch.nn.TransformerEncoderLayer(transformer_input_dims,  nhead=transformer_head_count, dim_feedforward=transformer_linear_dim, activation=transformer_activiation_func, norm_first=transformer_layer_norm_first, dropout=transformer_dropout, batch_first=True)
        decoder_norm = None
        if use_layer_norm == True: decoder_norm = torch.nn.LayerNorm(transformer_input_dims)
        self.transformer = torch.nn.TransformerEncoder(decoder_layer, num_layers=transformer_layer_count, norm=decoder_norm) #nn.Transformer(transformer_input_dims, nhead=transformer_head_count, dim_feedforward=transformer_linear_dim, activation=transformer_activiation_func, num_decoder_layers=transformer_layer_count, num_encoder_layers=transformer_layer_count, norm_first=transformer_layer_norm_first, dropout=transformer_dropout, batch_first=True)
        ...
        if mode == "regressive" or mode == "auto-regressive":
            ...
            for index in range(math.ceil(prediction_horizon / prediction_step)):
                ...
                if self.use_mask == None:
                    ...
                    step_output = self.transformer(past, mask=src_mask, is_causal=True)[..., -prediction_step:, :]
