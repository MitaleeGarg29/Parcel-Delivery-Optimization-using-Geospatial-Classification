...
from .internal.pew_lstm import Pew_LSTM
from .internal.model_util import model_util


# Expects data of type (input, input, label)
# Expects parameters of type (input, input)
class LSTM_V1(pl.LightningModule):
    def __init__(self, in_dim, weather_dim, input_dim_3, hidden_dim=10, window_size=1, pos_weight=2):
        super(LSTM_V1, self).__init__()
        self.lstm = Pew_LSTM(in_dim=in_dim, hidden_dim=hidden_dim, weather_dim=weather_dim, window_size=window_size)
        ...
        self.save_hyperparameters() #untested
