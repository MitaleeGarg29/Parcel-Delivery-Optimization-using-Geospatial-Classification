...
class singleuser_V1_c(dp):
    usr_id = 'CUSTOMER_94915209-47e3-460d-baa2-bbaae64d5e3c' #'4147127f-1c47-4de8-8e43-58f19d41b418'
    geo_hash_home = 'mr17x'
    ...
    def _process(self, loaded_raw_data, **kwargs) -> list[tuple[TensorDataset]]:
        ...
        # TODO: Where does 6 am come from? Is it even enforced?
        ...
        print(f"distance_2_home_np length: {len(distance_2_home_np)} and shape {distance_2_home_np.shape}")
        ...
        # TODO: Output y and see where/how dim changes, then try to understand all dim changes
        ...
        print(f"target length: {len(target)} and shape {target.shape}")
        ...
        #print(f"second x_input occurence length: {len(x_input)} and shape {x_input.shape}")
        TRAIN_PERCENT=80
        ...
        #print(f"third x_input occurence length: {len(x_input)} and shape {x_input.shape}")
        x_train_add=x_input_additional[:day_threshold]
        target_train=target[:day_threshold,...,0]
        print(f"target_train length: {len(target_train)} and shape {target_train.shape}")
        print(f"first input length: {len(x_train)} and shape {x_train.shape}")
        print(f"second input length: {len(x_train_add)} and shape {x_train_add.shape}")
        print(f"target length: {len(target_train)} and shape {target_train.shape}")
