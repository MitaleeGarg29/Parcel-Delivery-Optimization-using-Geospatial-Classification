...
class singleuser_V1(dp):
    usr_id = 'CUSTOMER_4147127f-1c47-4de8-8e43-58f19d41b418' #'4147127f-1c47-4de8-8e43-58f19d41b418'
    geo_hash_home = '3zdz6'

    def _load(self, **kwargs) -> str:
        
        urls = ['https://rl.green-convenience.com/api/v1/positions/convert/' + singleuser_V1.geo_hash_home,
                "https://rl.green-convenience.com/api/v1/positions/preprocess/" + singleuser_V1.usr_id + "/" + singleuser_V1.geo_hash_home]
    ...
    def _process(self, loaded_raw_data, **kwargs) -> list[tuple[TensorDataset]]:
        ...
        # TODO: IF THIS REMOVES ENTIRE ROWS THEN ENTIRE DAYS CAN BE REMOVED AND THE DAY RANGE WILL BE MODIFIED? If so, then it needs to be adjusted and filled with additional empty data to match desired date range (prob only at end and beginning)
        main_df = pd.DataFrame(json_data[1]).dropna(subset=['coordinate'])

        # Step 3: Convert the timestamp field to a datetime object
        main_df['datetime'] = pd.to_datetime(main_df['timestamp'], unit='ms').dt.round('15min') # TODO: Test if this actually rounds to 15 min
